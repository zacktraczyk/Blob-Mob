...
const faceGlasses: FaceFunction = (ctx: CanvasRenderingContext2D, faceAttr: FaceAttr) => {
  ...
  ctx.beginPath()
  ctx.fillStyle = 'white'
  ctx.arc(x + w / 3 + xdir / 2, y + h / 2 + ydir / 2, (w / 4 + h / 4) / 5, 0, 2 * Math.PI)
  ctx.fill()
  ctx.stroke()
  ctx.closePath()
  ...
  //Draws Right Eye
  ctx.beginPath()
  ctx.arc(x + w * (2 / 3) + xdir / 2, y + h / 2 + ydir / 2, (w / 4 + h / 4) / 5, 0, 2 * Math.PI)
  ctx.fill()
  ctx.stroke()
  ctx.closePath()
