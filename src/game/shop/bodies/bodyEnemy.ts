...
const bodyEnemy: BodyFunction = (ctx: CanvasRenderingContext2D, bodyAttr: BodyAttr) => {
  const { h, cool, damaging } = bodyAttr
  let { x, y, w } = bodyAttr
  ...
  //Draws Body
  ctx.moveTo(x - w / 8, y)
  ctx.bezierCurveTo(x - w / 8, y - h / 4, x + w + w / 8, y - h / 4, x + w + w / 8, y)

  ctx.bezierCurveTo(x + w * 2, y, x + w * 2, y + h, x + w - w / 8, y + h)

  ctx.bezierCurveTo(x + w - w / 8, y + h * 1.75, x - w * 2, y + h / 4, x - 10, y + h / 2 - 10)
